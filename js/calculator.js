...
//for blocking GH5 aspect ratio changes
function checkGH5(e) {
    if (document.getElementById(e.target.id).value == 2.081) {
        //lock sensor aspect ratio at 4:3
        document.getElementById("sar").disabled = true;
        document.getElementById("sar").value = 1.33;
    }
    else {
        //unlock sensor and proceed normally
        document.getElementById("sar").disabled = false;
        document.getElementById("sar").value = 1.78;
        checkCustom(e);
    }
}
...
//gets string input, replaces all commas with dots and returns a number
...
function willVignette(hfov) {
    ...
    if (baby === true) {
        ...
        //partial vignette
                ...
                (stretch === 2.001 && (fov >= 60 && fov < 67.5))) { //sankor, hifi
                    return "Very likely show dark corners";
        }
        //no data
        else if (anaArray.indexOf(stretch) == -1) {
...
function getData() {
    var tklens, crop, reducer, ana, sar, sarB, hfov, rar, sf, focus, iris, backsize;
...
function calculateHFOV() {
    ...
    if (checkArray(inputData)) {
        ...
        //do the math
        hfov = Math.round(((crop * reducer * tklens * (1/ana)) / sarB));
...
function calculateTkLens() {
    ...
    if (checkArray(inputData)) {
        ...
        var tklens = Math.round(hfov / (crop * reducer * 1/ana / sarB) / (1-sfWide));
...
function saveToURL() {
    ...
    params.set("tklens", document.getElementById("tklens").value);
    params.set("hfov", document.getElementById("hfov").value);
    params.set("rar", document.getElementById("rar").value);
    params.set("backsize", document.getElementById("backsize").value);
    params.set("focus", document.getElementById("focus").value);
    params.set("crop", document.getElementById("crop").value);
    params.set("reducer", document.getElementById("reducer").value);
    params.set("ana", document.getElementById("ana").value);
    params.set("sar", document.getElementById("sar").value);
    params.set("sf", document.getElementById("sf").value);
    params.set("pancake", document.getElementById("pancake").checked ? "1" : "0");
    params.set("baby", document.getElementById("baby").checked ? "1" : "0");
    if (document.getElementById("crop").value === "custom" && document.getElementById("customcrop")) {
        params.set("customcrop", document.getElementById("customcrop").value);
    }
    if (document.getElementById("reducer").value === "custom" && document.getElementById("customreducer")) {
        params.set("customreducer", document.getElementById("customreducer").value);
    }
    if (document.getElementById("ana").value === "custom" && document.getElementById("customana")) {
        params.set("customana", document.getElementById("customana").value);
    }
    if (document.getElementById("sar").value === "custom" && document.getElementById("customsar")) {
        params.set("customsar", document.getElementById("customsar").value);
    }
...
function loadFromURL() {
    ...
    if (!params.has("tklens")) return;

    // Restore plain inputs
    ["tklens", "hfov", "rar", "backsize"].forEach(function(id) {
        if (params.has(id)) document.getElementById(id).value = params.get(id);
    });

    // Restore checkboxes before schematic init so baby/pancake state is correct
    if (params.has("pancake")) document.getElementById("pancake").checked = params.get("pancake") === "1";
    if (params.has("baby")) document.getElementById("baby").checked = params.get("baby") === "1";

    // Restore dropdowns and handle custom values
    ["crop", "reducer", "ana", "sar"].forEach(function(id) {
        if (params.has(id)) {
            var val = params.get(id);
            document.getElementById(id).value = val;
            if (val === "custom") {
                checkCustom({ target: { id: id } });
                var customId = "custom" + id;
                if (params.has(customId) && document.getElementById(customId)) {
                    document.getElementById(customId).value = params.get(customId);
                }
            }
        }
    });

    // Re-apply GH5 SAR lock if needed
    if (document.getElementById("crop").value == "2.081") {
        document.getElementById("sar").disabled = true;
        document.getElementById("sar").value = 1.33;
    }

    // Restore single focus solution and slider visibility
    if (params.has("sf")) {
        document.getElementById("sf").value = params.get("sf");
        sfSlider({ target: { id: "sf" } });
    }

    // Restore focus slider position
    if (params.has("focus")) document.getElementById("focus").value = params.get("focus");

    // Redraw schematic with restored state, then calculate
    if (window.schematic) {
        window.schematic.init();
        window.schematic.draw();
    }
    calculateHFOV();
}
...
document.addEventListener("keydown", function(e) {
    if (e.key === "Enter") {
        ...
        if (document.getElementById("section-shape-1").contains(active)) {
            calculateHFOV();
        } else if (document.getElementById("section-shape-2").contains(active)) {
            calcDiopter();
        }
    }
});
